...
static void I2C_Mode_Config(void)
{
	...
	I2C_Cmd(AT24C02_I2C, ENABLE); // 使能I2C1
	I2C_AcknowledgeConfig(AT24C02_I2C, ENABLE);
}

/**
 * @brief  I2C外设(EEPROM)初始化
 * @param  无
 * @retval 无
 */
 ...
 *          E2PROM在执行写入擦除等命令后，需一定时间才可执行完，
 *          在这过程中不可接收新操作。此函数用于等待E2PROM内部执行完毕
 * @param  无
 * @retval 无
 */
void AT24C02_WaitE2promOK(void)
...
int8_t AT24C02_BufferRead(uint8_t* pBuffer, uint8_t ReadAddr, uint16_t Number)
{
	...
	I2C_SendData(AT24C02_I2C, ReadAddr); // 发送要读取数据的地址值
