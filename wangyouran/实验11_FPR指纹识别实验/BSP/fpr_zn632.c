...
uint32_t i = 0;
GPIO_SetBits(GPIOC, GPIO_Pin_9);     //先拉高
ZN632_DELAY_MS(100);
GPIO_ResetBits(GPIOC, GPIO_Pin_9);    //拉低加电

//等待0x55，同时轮询UART4 RXNE
while(1){
if(USART_GetFlagStatus(ZN632_USART, USART_FLAG_RXNE) != RESET){
uint8_t ch = USART_ReceiveData(ZN632_USART);
if(ch == 0x55){//启动成功标志
ZN632_ClearRxBuf();
return 0;
}
...
ZN632_RxBuf[ZN632_RxBufLen++] = USART_ReceiveData( UART4 );
...
for(i=0; i<500; i++){
if(USART_GetFlagStatus(ZN632_USART, USART_FLAG_RXNE) != RESET){
ZN632_RxBuf[ZN632_RxBufLen++] = USART_ReceiveData(ZN632_USART);
}
ZN632_DELAY_MS(1);
}
...
ret = ZN632_CheckAck();  //内部有500ms轮询等待
if(ret != 0){//显示错误信息
ZN632_ShowError(ret);
}
