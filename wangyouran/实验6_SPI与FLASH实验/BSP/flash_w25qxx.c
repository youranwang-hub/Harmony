...
uint32_t W25QXX_ReadID(void)
{
	...
	Temp = (Temp0 << 16) | (Temp1 << 8) | Temp2; // 拼接数据
	return Temp;
}
...
void W25QXX_WaitForWriteEnd(void)
{
	...
	W25QXX_SendByte(CMD_READ_STATUS_REG); // 读状态寄存器命令
...
void W25QXX_PageWrite(u8* pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
	...
	W25QXX_SendByte(CMD_PAGE_PROGRAM);                    // 发送页写指令
...
/**
 * @brief  将缓冲区中的数据写到W25QXX中（自动处理跨页）
 * @param  pBuffer:         要写入数据的指针
 * @param  WriteAddr:       写入地址
...
void W25QXX_BufferRead(u8* pBuffer, uint32_t ReadAddr, uint16_t NumByteToRead)
{
	...
	W25QXX_SendByte(CMD_READ_DATA);                       // 发送读指令
	W25QXX_SendByte((ReadAddr & 0xFF0000) >> 16);          // 发送读地址高位
	W25QXX_SendByte((ReadAddr & 0xFF00) >> 8);             // 发送读地址中位
	W25QXX_SendByte(ReadAddr & 0xFF);                      // 发送读地址低位
