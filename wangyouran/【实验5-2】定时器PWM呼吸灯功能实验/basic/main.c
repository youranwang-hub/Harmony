...
TIM_OC2PreloadConfig(TIM3, TIM_OCPreload_Enable);//使能TIM3在CCR2预装载寄存器
...
TIM_SetCompare2(TIM3,0); //写入PWM值(0--499)
	...
	PWM_Value = 250;                //初始半亮，KEY1增亮 KEY2变暗
...
while(1)
{
	...
	{
		PWM_Value += 50;                 //每次步进50，亮度变化明显
		if(PWM_Value > 499) PWM_Value = 499; //上限499
		TIM_SetCompare2(TIM3, PWM_Value);
		while(0 == GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0)); //等待按键抬起
	}
	if(0 == GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_13))//按键2被按下
	{
		if(PWM_Value >= 50) PWM_Value -= 50; //每次步进50
		else                 PWM_Value = 0;   //接近0直接关
		TIM_SetCompare2(TIM3, PWM_Value);
		while(0 == GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_13)); //等待按键抬起
	}
