...
// 屏幕尺寸全局变量，和头文件声明对应
uint16_t g_LCD_Width  = ILI9341_LESS_PIX;   // 240
uint16_t g_LCD_Height = ILI9341_LONG_PIX;   // 320
...
//==================== 新增封装函数，无函数名冲突 ====================
// 全屏清屏函数：LCD_ClearFull(颜色)，解决原同名冲突报错
void LCD_ClearFull(uint16_t color)
{
    LCD_SetBackColor(color);
    LCD_Clear(0, 0, g_LCD_Width, g_LCD_Height);
}
