# 外围实验9-1拓 XPT2046 触摸屏拓展

## 1. 实验目标

本实验属于 **XPT 拓展版**，当前版本为 **拓展版**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。

- 掌握 XPT2046 区域触摸 的基础操作。
- 理解模块初始化、数据交互和结果显示流程。
- 能够通过串口或 LCD 验证实验结果。
...
```text
D:\Github\Harmony\zhangjiayuan\（ARM外围传感器实验）实验9_XPT触摸屏实验\【外围实验9-1拓】XPT触摸屏
```
...
| 项目 | 内容 |
|---|---|
| `代码/BSP/xpt2046.c` | 模块实现文件 |
| `代码/BSP/xpt2046.h` | 头文件/接口声明 |
...
| 项目 | 内容 |
|---|---|
| `XPT2046_Init()` | 位于 `代码/BSP/xpt2046.c`，用于初始化、数据处理或功能控制 |
| `XPT2046_DetectTouch()` | 位于 `代码/BSP/xpt2046.c`，用于初始化、数据处理或功能控制 |
| `XPT2046_TouchDown()` | 位于 `代码/USER/main.c`，用于初始化、数据处理或功能控制 |
| `XPT2046_TouchUp()` | 位于 `代码/USER/main.c`，用于初始化、数据处理或功能控制 |
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | 点击不同区域触发不同显示或操作。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |

若使用串口观察，建议串口助手参数与源码保持一致；若使用显示屏观察，优先确认显示模块基础初始化正常。

## 9. 基础版与拓展版差异

拓展版相对基础版强调触摸区域命中和页面交互。
