...
void gga_parse(GGA* gga, char *gga_data){
    ...
    while(p){
        ...
        switch(times){
            ...
            case 6:   // quality
                gga->quality =
                    (unsigned char)strtol(p, NULL, 10);
                break;
            ...
            case 14:  // stn_ID
                gga->stn_ID =
                    (unsigned short)strtol(p, NULL, 10);
                break;
...
void gsa_show(GSA *gsa){
    ...
    /*
    mode_MA保存的是A或M，应使用%c显示
    */
    printf("mode_MA:%c\r\n", gsa->mode_MA);

    /*
    mode_123保存的是字符1、2、3，应使用%c显示
    */
    printf("mode_123:%c\r\n", gsa->mode_123);
...
void gsv_parse(GSV *gsv, char *gsv_data){
    ...
    while(p){
        ...
        switch(times){
            ...
            case 6:   //azimuth
                if(sat_index < 36){
                    gsv->sat_info[sat_index].azimuth =
