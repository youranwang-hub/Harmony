...
本实验属于 **PWM 输出实验**，当前版本为 **基础版**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。
...
```text
D:\Github\Harmony\zhangjiayuan\【实验5-2】呼吸灯
```
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | LED 亮度由暗到亮再由亮到暗，形成呼吸灯效果。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |
...
| 问题 | 可能原因与处理方法 |
|---|---|
| 亮度变化不明显 | 调整 PWM 周期和占空比变化步进。 |
| LED 不亮 | 检查 PWM 复用引脚和通道。 |
