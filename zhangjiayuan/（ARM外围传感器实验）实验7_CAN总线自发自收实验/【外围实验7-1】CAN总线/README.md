...
本实验属于 **CAN 基础版**，当前版本为 **基础版**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。
...
```text
D:\Github\Harmony\zhangjiayuan\（ARM外围传感器实验）实验7_CAN总线自发自收实验\【外围实验7-1】CAN总线
```
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | 串口输出发送成功和接收到的 ID/数据。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |
