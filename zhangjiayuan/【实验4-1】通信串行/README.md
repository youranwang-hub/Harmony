...
本实验属于 **串口通信实验**，当前版本为 **基础版**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。
...
```text
D:\Github\Harmony\zhangjiayuan\【实验4-1】通信串行
```
...
| 项目 | 内容 |
|---|---|
| `main()` | 位于 `4mainplus1.c`，用于初始化、数据处理或功能控制 |
| `LED_Init()` | 位于 `4mainplus1.c`，用于初始化、数据处理或功能控制 |
| `USART2_Init()` | 位于 `4mainplus1.c`，用于初始化、数据处理或功能控制 |
| `USART2_Send_String()` | 位于 `4mainplus1.c`，用于初始化、数据处理或功能控制 |
| `Command_Match()` | 位于 `4mainplus1.c`，用于初始化、数据处理或功能控制 |
| `Flow_LED_Run()` | 位于 `4mainplus1.c`，用于初始化、数据处理或功能控制 |
| `delay_ms()` | 位于 `4mainplus1.c`，用于初始化、数据处理或功能控制 |
| `USART2_IRQHandler()` | 位于 `4mainplus1.c`，用于初始化、数据处理或功能控制 |
| `main()` | 位于 `main.c`，用于初始化、数据处理或功能控制 |
| `USART2_Send_Frame()` | 位于 `main.c`，用于初始化、数据处理或功能控制 |
| `delay_us()` | 位于 `main.c`，用于初始化、数据处理或功能控制 |
| `delay_ms()` | 位于 `main.c`，用于初始化、数据处理或功能控制 |
| `USART2_IRQHandler()` | 位于 `main.c`，用于初始化、数据处理或功能控制 |

### 5.3 实现要点

- 配置 TX/RX 引脚复用。
- 设置波特率、数据位、停止位和校验方式。
- 通过 `printf` 或发送函数输出调试信息。
- 解析串口接收命令并执行 LED 控制。
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | 串口助手能接收板端输出。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |
| 运行现象 2 | 输入指定命令后 LED 状态改变或串口返回确认。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |
