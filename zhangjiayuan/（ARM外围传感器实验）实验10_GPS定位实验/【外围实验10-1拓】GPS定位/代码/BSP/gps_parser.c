...
/* 节转换为千米每小时 */
#define GPS_KNOT_TO_KMH       1.852

/* 运动状态判断阈值 */
#define GPS_STOP_SPEED_KMH    1.0
#define GPS_LOW_SPEED_KMH     10.0
...
经度格式：dddmm.mmmm
*/
static double gps_dm_to_degree(double dm, char direction)
{
    int degree;
    double minute;
    double result;
...
判断是否为闰年
*/
static int gps_is_leap_year(int year)
{
    if((year % 400) == 0){
        return 1;
    }
...
date格式：DDMMYY
*/
static void gps_utc_to_beijing(
    const char *utc,
    const char *date,
    int *year,
    int *month,
    ...
    int *second)
{
    ...
    *second = 0;
    ...
    if(utc_hour >= 24){
        ...
        month_days = gps_days_in_month(utc_year, utc_month);
...
将节转换为千米每小时
*/
static double gps_knots_to_kmh(double speed_knots)
{
    return speed_knots * GPS_KNOT_TO_KMH;
}

/*
根据速度判断运动状态
*/
static const char *gps_get_motion_state(double speed_kmh)
{
    if(speed_kmh < GPS_STOP_SPEED_KMH){
        return "STATIONARY";
    }

    if(speed_kmh < GPS_LOW_SPEED_KMH){
        return "LOW SPEED";
    }
...
void rmc_show(RMC *rmc){
    ...
    拓展1：经纬度转换为十进制度
    */
    lat_degree = gps_dm_to_degree(rmc->lat, rmc->lat_dir);
    lon_degree = gps_dm_to_degree(rmc->lon, rmc->lon_dir);

    printf("---------------EXTENDED DATA:\r\n");
    printf("lat_degree:%.6lf\r\n", lat_degree);
    printf("lon_degree:%.6lf\r\n", lon_degree);
    ...
    */
    ...
    if(year != 0){
        printf("beijing_time:%04d-%02d-%02d %02d:%02d:%02d\r\n",
               year,
               month,
               day,
               hour,
        ...
        printf("beijing_time:INVALID\r\n");
    }

    /*
    拓展3：速度换算和运动状态判断
    */
    speed_kmh = gps_knots_to_kmh(rmc->speed_Kn);
    motion_state = gps_get_motion_state(speed_kmh);
