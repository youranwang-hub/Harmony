...
本实验属于 **I2C 存储实验**，当前版本为 **基础/拓展**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。

- 掌握 I2C 总线读写流程。
- 完成 AT24C02 指定地址写入和读回。
- 理解 EEPROM 掉电保存和写周期。
...
```text
D:\Github\Harmony\zhangjiayuan\（ARM外围传感器实验）实验5_I2C与E2PROM实验
```
...
| `代码/BSP/e2prom_at24c02.c` | 模块实现文件 |
| `代码/BSP/e2prom_at24c02.h` | 头文件/接口声明 |
...
| `AT24C02_Init()` | 位于 `代码/BSP/e2prom_at24c02.c`，用于初始化、数据处理或功能控制 |
| `AT24C02_WaitE2promOK()` | 位于 `代码/BSP/e2prom_at24c02.c`，用于初始化、数据处理或功能控制 |
| `AT24C02_BufferWrite()` | 位于 `代码/BSP/e2prom_at24c02.c`，用于初始化、数据处理或功能控制 |
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | 串口显示写入成功、读取数据一致或校验通过。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |

若使用串口观察，建议串口助手参数与源码保持一致；若使用显示屏观察，优先确认显示模块基础初始化正常。

## 9. 基础版与拓展版差异

拓展内容可增加多字节写入、跨页处理或掉电保存验证。

## 10. 常见问题与调试建议

| 问题 | 可能原因与处理方法 |
|---|---|
| 写后读不一致 | 检查写周期延时和地址是否正确。 |
| I2C 无应答 | 检查 SCL/SDA、上拉和器件地址。 |
