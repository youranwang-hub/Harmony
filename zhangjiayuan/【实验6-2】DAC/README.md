...
本实验属于 **DAC 实验**，当前版本为 **基础版**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。
...
```text
D:\Github\Harmony\zhangjiayuan\【实验6-2】DAC
```
...
| 项目 | 内容 |
|---|---|
| `main()` | 位于 `6mainplus2.c`，用于初始化、数据处理或功能控制 |
| `delay_us()` | 位于 `6mainplus2.c`，用于初始化、数据处理或功能控制 |
| `delay_ms()` | 位于 `6mainplus2.c`，用于初始化、数据处理或功能控制 |
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | 改变 DAC 数据后，输出电压随之变化。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |
