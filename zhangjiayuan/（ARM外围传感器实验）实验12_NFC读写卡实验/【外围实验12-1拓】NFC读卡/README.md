...
本实验属于 **NFC 拓展版**，当前版本为 **拓展版**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。
...
```text
D:\Github\Harmony\zhangjiayuan\（ARM外围传感器实验）实验12_NFC读写卡实验\【外围实验12-1拓】NFC读卡
```
...
| 项目 | 内容 |
|---|---|
| `代码/BSP/nfc_pn532.c` | 模块实现文件 |
| `代码/BSP/nfc_pn532.h` | 头文件/接口声明 |
...
| 项目 | 内容 |
|---|---|
| `NFC_Init()` | 位于 `代码/BSP/nfc_pn532.c`，用于初始化、数据处理或功能控制 |
| `PN532_SendData()` | 位于 `代码/BSP/nfc_pn532.c`，用于初始化、数据处理或功能控制 |
| `USART1_IRQHandler()` | 位于 `代码/BSP/nfc_pn532.c`，用于初始化、数据处理或功能控制 |
| `PN532_ClearRxBuf()` | 位于 `代码/BSP/nfc_pn532.c`，用于初始化、数据处理或功能控制 |
| `PN532_CheckSum()` | 位于 `代码/BSP/nfc_pn532.c`，用于初始化、数据处理或功能控制 |
| `make_test_data()` | 位于 `代码/USER/main.c`，用于初始化、数据处理或功能控制 |
| `print_card_uid()` | 位于 `代码/USER/main.c`，用于初始化、数据处理或功能控制 |
| `print_block_data()` | 位于 `代码/USER/main.c`，用于初始化、数据处理或功能控制 |
| `clear_read_buffer()` | 位于 `代码/USER/main.c`，用于初始化、数据处理或功能控制 |
| `main()` | 位于 `代码/USER/main.c`，用于初始化、数据处理或功能控制 |

### 5.3 实现要点

- 在读 UID 基础上增加认证、数据块读取或写入。
- 写入后再次读取进行校验。
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | 串口显示读写成功和数据一致。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |
