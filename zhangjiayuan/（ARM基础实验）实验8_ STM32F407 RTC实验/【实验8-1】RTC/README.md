...
本实验属于 **RTC 时间实验**，当前版本为 **基础版**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。
...
```text
D:\Github\Harmony\zhangjiayuan\（ARM基础实验）实验8_ STM32F407 RTC实验\【实验8-1】RTC
```
...
| `Write_IIC_Data()` | 位于 `代码/BSP/oled.c`，用于初始化、数据处理或功能控制 |
| `OLED_WR_Byte()` | 位于 `代码/BSP/oled.c`，用于初始化、数据处理或功能控制 |
| `fill_picture()` | 位于 `代码/BSP/oled.c`，用于初始化、数据处理或功能控制 |
| `OLED_Set_Pos()` | 位于 `代码/BSP/oled.c`，用于初始化、数据处理或功能控制 |
| `OLED_Display_On()` | 位于 `代码/BSP/oled.c`，用于初始化、数据处理或功能控制 |
| `OLED_Display_Off()` | 位于 `代码/BSP/oled.c`，用于初始化、数据处理或功能控制 |
| `OLED_Clear()` | 位于 `代码/BSP/oled.c`，用于初始化、数据处理或功能控制 |
| `OLED_On()` | 位于 `代码/BSP/oled.c`，用于初始化、数据处理或功能控制 |
| `OLED_ShowChar()` | 位于 `代码/BSP/oled.c`，用于初始化、数据处理或功能控制 |
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | 时间按秒递增，显示或串口输出不断更新。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |
...
| 问题 | 可能原因与处理方法 |
|---|---|
| 时间不走 | 检查 RTC 时钟源和初始化是否成功。 |
| 时间重置 | 检查备份域和是否每次都重新初始化。 |
