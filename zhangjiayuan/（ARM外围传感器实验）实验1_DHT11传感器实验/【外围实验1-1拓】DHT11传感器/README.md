# 外围实验1-1拓 DHT11 温湿度拓展

## 1. 实验目标

本实验属于 **DHT11 拓展版**，当前版本为 **拓展版**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。

- 掌握 DHT11 + 显示/报警 的基本使用。
- 理解数据采集、解析和输出流程。
- 能够通过串口或显示设备验证实验结果。
...
```text
D:\Github\Harmony\zhangjiayuan\（ARM外围传感器实验）实验1_DHT11传感器实验\【外围实验1-1拓】DHT11传感器
```
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | 显示界面或串口持续刷新温湿度。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |

若使用串口观察，建议串口助手参数与源码保持一致；若使用显示屏观察，优先确认显示模块基础初始化正常。

## 9. 基础版与拓展版差异

拓展版相对基础版增加数据显示或状态提示，重点体现传感器数据如何进入人机界面。
