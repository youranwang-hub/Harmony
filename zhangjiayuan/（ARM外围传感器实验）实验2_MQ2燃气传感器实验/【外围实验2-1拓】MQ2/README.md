...
本实验属于 **MQ2 拓展版**，当前版本为 **拓展版**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。

- 掌握 MQ-2 + ADC + 报警 的基本使用。
- 理解数据采集、解析和输出流程。
- 能够通过串口或显示设备验证实验结果。
...
```text
D:\Github\Harmony\zhangjiayuan\（ARM外围传感器实验）实验2_MQ2燃气传感器实验\【外围实验2-1拓】MQ2
```
...
|---|---|
| `代码/BSP/mq.c` | 模块实现文件 |
| `代码/BSP/mq.h` | 头文件/接口声明 |
| `代码/BSP/usart.c` | 模块实现文件 |
| `代码/BSP/usart.h` | 头文件/接口声明 |
| `代码/SYSTEM/delay.c` | 模块实现文件 |
| `代码/SYSTEM/delay.h` | 头文件/接口声明 |
...
| 项目 | 内容 |
|---|---|
| `MQ_Init()` | 位于 `代码/BSP/mq.c`，用于初始化、数据处理或功能控制 |
| `MQ_ReadValue()` | 位于 `代码/BSP/mq.c`，用于初始化、数据处理或功能控制 |
| `MQ_ReadAverage()` | 位于 `代码/BSP/mq.c`，用于初始化、数据处理或功能控制 |
| `MQ_CalibrateBaseline()` | 位于 `代码/BSP/mq.c`，用于初始化、数据处理或功能控制 |
| `MQ_ADCToMillivolt()` | 位于 `代码/BSP/mq.c`，用于初始化、数据处理或功能控制 |
...
| `delay_us()` | 位于 `代码/SYSTEM/delay.c`，用于初始化、数据处理或功能控制 |
| `delay_ms()` | 位于 `代码/SYSTEM/delay.c`，用于初始化、数据处理或功能控制 |
| `main()` | 位于 `代码/USER/main.c`，用于初始化、数据处理或功能控制 |

### 5.3 实现要点

- 在基础 ADC 读取基础上增加阈值判断。
- 超过阈值时触发 LED、蜂鸣器或显示提示。
- 保留串口输出便于调试。
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | 气体值超过阈值后出现报警提示。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |

若使用串口观察，建议串口助手参数与源码保持一致；若使用显示屏观察，优先确认显示模块基础初始化正常。

## 9. 基础版与拓展版差异

拓展版增加阈值判断和报警输出，用于验证传感器数据到控制输出的闭环。
