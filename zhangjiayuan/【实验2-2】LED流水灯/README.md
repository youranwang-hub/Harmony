...
本实验属于 **GPIO 顺序控制实验**，当前版本为 **基础版**。实验围绕 STM32F407 标准外设库展开，重点训练单个模块从初始化、运行到结果验证的完整流程。

- 掌握多个 GPIO 输出的组合控制。
- 理解主循环与延时函数配合形成可见动态效果。
- 实现 LED 顺序点亮和熄灭。
...
|---|---|---|
| LED1/LED2/LED3 | GPIO 输出 | 顺序亮灭形成流水灯 |
...
```text
D:\Github\Harmony\zhangjiayuan\【实验2-2】LED流水灯
```
...
| `main()` | 位于 `mainplus2.c`，用于初始化、数据处理或功能控制 |
| `delay_us()` | 位于 `mainplus2.c`，用于初始化、数据处理或功能控制 |
| `delay_ms()` | 位于 `mainplus2.c`，用于初始化、数据处理或功能控制 |
...
| 验证项目 | 预期现象 | 判断依据 |
|---|---|---|
| 运行现象 1 | 多个 LED 依次点亮、熄灭，形成连续流水效果。 | 现象稳定出现，说明对应初始化和控制逻辑有效 |
